...
STOR=$(echo $STORAGE|sed -e 's/\//\\\//g')
FILES=$(ls ./www/cgi-bin)
sed -i -e "s/http:\/\/localhost/http:\/\/$SITENAME/" ./www/index.html
for I in $FILES
do
	echo -n .
     sed -i -e "s/http:\/\/localhost/http:\/\/$SITENAME/" ./www/cgi-bin/$I
done
echo
...
echo -n .
sed -i -e "s/##SERVERADDRESS##/$SERVADDR/" ./www/cgi-bin/reg.sh
echo -n .
sed -i -e "s/##SERVERADDRESS##/$SERVADDR/" ./www/cgi-bin/getbat.pl
echo -n .
